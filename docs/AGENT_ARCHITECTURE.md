...
| **Validator Agent** | `Capability` + retrieved evidence (top-k from vector index) | `list[Contradiction]` | `claude-sonnet-4-6` | 1 | n/a | Runs on EVERY extraction (not on-demand); cross-checks against staff_roster / equipment_inventory / volume_report sources; uses `ContradictionType` enum |
