...
**Generated:** 2026-04-26 — Phase 0 baseline freeze.
**Source plan:** `docs/PRODUCT_READINESS_AUDIT_PROMPT.md`.
**Branch hash:** `21cac84` (`integrate/ship-12h`).
**Test count:** **279 passing**, `pytest -q` clean from a fresh worktree.
...
| 1.4 | Medical deserts rendered geographically with planner actions | **Partial** | `/map/aggregates` returns 8 regions, but the UI renders a placeholder choropleth; needs Lane 3C work. |
...
| 3.1 | Dashboard / Map / Planner / Audit run against live API | **Partial — foundation only** | Phase 3A shipped `app/src/api/client.ts` (typed fetcher for `/summary`, `/query`, `/facilities/{id}`, `/map/aggregates`, `/health/data`) and `app/src/types/api.ts` (TS shapes). The four pages still import `app/src/data/demoData.ts` directly. **Residual gap:** the `VITE_SEAHEALTH_API_MODE` env var does not yet steer the visible UI; it only steers anyone who opts into `client.ts`. Migration is a Phase 8 follow-up. |
| 3.2 | Demo data is deliberate offline mode, not hidden dependency | **Partial — overclaim removed** | `app/src/data/demoData.ts` is still imported directly in pages. The README, demo script, and one-pager now say so explicitly rather than promising the migration is done. The fixtures bundled at `app/src/data/fixtures/` are the API-shape demo data the client uses; the static `demoData.ts` is the visible-UI demo data. |
| 3.3 | Every recommendation shows evidence + contradiction + confidence | **Yes (in demo path)** | The visible UI's facility audit page renders trust score, contradictions with severity + reasoning, and evidence cards. Phase 3B added `TraceClassBadge` (live/synthetic/missing) for the trace-quality state, but it has not been wired into the page yet — it is shipped as an importable component for the next pass. |
...
| 5.1 | README quickstart works on clean clone | **Partial** | README exists. Test count line says "246 tests" (drift; actually 279). **Gap:** verify the four startup paths, document live-mode prereqs, document fixture mode. |
