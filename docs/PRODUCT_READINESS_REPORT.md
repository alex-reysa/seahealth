...
| 2.3 | OpenRouter / direct LLM use framed as decision, not hidden | **Partial** | `.env.example` and `DECISIONS.md` are explicit. **Gap:** README and one-pager need to surface this honestly. |
| 3.1 | Dashboard / Map / Planner / Audit run against live API | **Partial** | All four pages exist (`Dashboard.tsx`, `DesertMap.tsx`, `PlannerQuery.tsx`, `FacilityAudit.tsx`) but `Map Workbench` route layout overlays demo data. **Gap:** typed API client, `VITE_SEAHEALTH_API_MODE` switch, error/loading/empty states (Lane 3A). |
| 3.2 | Demo data is deliberate offline mode, not hidden dependency | **Partial** | `app/src/data/demoData.ts` is imported directly in pages today. **Gap:** route everything through a single data provider that picks live vs. demo (Lane 3A). |
| 3.3 | Every recommendation shows evidence + contradiction + confidence | **Partial** | API payloads carry all three; UI shows trust score badges and contradiction count, but evidence snippets and trace IDs are not rendered together yet (Lane 3B). |
