...
- **2026-04-26 — Phase 1A: `classify_trace_id` + `build_audits` trace breakdown.** New helper `seahealth.agents.facility_audit_builder.classify_trace_id(trace_id) -> Literal["live", "synthetic", "missing"]` distinguishes real MLflow trace ids (no `local::` prefix), the deterministic synthetic ids written by `seahealth.pipelines.extract` when `MLFLOW_TRACKING_URI` is unset, and `None`/empty values. `seahealth.pipelines.build_audits.main` now returns `live_trace_count`, `synthetic_trace_count`, and `missing_trace_count` alongside `audit_count`, and logs a warning when no audits carry a live trace id. Behavior when MLflow is unavailable: extraction stamps `Capability.mlflow_trace_id = local::<facility_id>::<run_uuid>`; the audit inherits that string and `classify_trace_id` returns `"synthetic"`. The UI is expected to render synthetic ids as a "trace not available — ran without MLflow" state, not as a clickable link.
- **2026-04-26 — Phase 1B: citation QA report + span classifier.** New module `seahealth.eval.citations_qa` walks every `EvidenceRef.snippet` against `tables/chunks.parquet` and classifies it as `valid_substring` (snippet appears verbatim), `valid_normalized` (matches after whitespace collapse), `empty_snippet`, `chunk_missing`, or `mismatch` (covers (0,0) extractor failure markers and genuine snippet-absent cases). CLI: `python -m seahealth.eval.citations_qa --capabilities tables/capabilities.parquet --chunks tables/chunks.parquet [--details out/citations.json]`. The classifier mirrors the extractor's anchoring (verbatim → normalized → fail), so a "valid" ref is exactly one a fresh extraction would accept; the report tells planners how many citation chips render real evidence vs. how many would render unsupported snippets.
